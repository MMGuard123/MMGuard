...
def decompose_single_apk(in_app, out_dir, with_res=True, with_src=True):
	cmd = 'java -jar apktool_2.4.1.jar d -f ' + in_app + ' -o ' + out_dir #change
	if not with_res:
		cmd += ' --no-res'
	if not with_src:
		cmd += ' --no-src'
...
def decompose_apks_category(in_dir, out_dir, with_res=True, with_src=True):
	...
	# decompose_apks(in_dir, out_dir, with_res=with_res, with_src=with_src)

# decompose_apks_category(RAW_APK_PATH, DECOMPOSED_APK_PATH, with_res=True,with_src=True)